...
The two-compose-file model keeps scopes clean: per-env services run under `<prefix>-<env>` with ports auto-derived from `WINTER_PORT_BASE` via `${WSD_PORT_<NAME>}` placeholders, so each named environment gets unique host ports without collision. Workspace singletons run once under `<prefix>-workspace`, using fixed ports or `${WINTER_WORKSPACE_PORT_BASE}`. `<prefix>` is `WINTER_SERVICE_PREFIX` — the workspace's `service_prefix`, resolved by winter-cli core and injected on every dispatch action. This guide does not configure the prefix; it's controlled entirely by the workspace, not per-provider.
...
**If `config.toml` already exists**, report what you found — `environment_compose_file`, `workspace_compose_file`, and the declared services. Do not scaffold. Tell the user: "`config.toml` already exists with `<n>` service(s) declared (`<name-list>`). I'll edit the existing files."

### Wire per-env services

**Explain first:** "Now I'll wire the per-env services (scope `"project"`) into `environment-compose.yaml` and declare them in `config.toml`. Per-env services run under `<prefix>-<env>` — one isolated compose project per feature env. Published host ports must use `${WSD_PORT_<NAME>}` placeholders (where `<NAME>` is the upper-cased service name); the orchestrator substitutes `WINTER_PORT_BASE + <position>` at runtime, where `<position>` is the 0-based declaration order among project-scoped `[[service]]` entries. This ensures no two named environments use the same host port."
