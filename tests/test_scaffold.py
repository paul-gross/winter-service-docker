...
class TestScaffoldFunction:
    ...
    def test_writes_compose_yaml(self, tmp_path: Path) -> None:
    ...
    def test_compose_yaml_uses_wsd_port_substitution(self, tmp_path: Path) -> None:
        scaffold(tmp_path)
        content = (tmp_path / "compose.yaml").read_text()
        assert "${WSD_PORT_" in content, "compose.yaml must use ${WSD_PORT_*} placeholders"

    def test_compose_yaml_has_named_volume(self, tmp_path: Path) -> None:
        scaffold(tmp_path)
        content = (tmp_path / "compose.yaml").read_text()
        assert "volumes:" in content, "compose.yaml must declare a named volume section"
    ...
    def test_config_toml_has_compose_file(self, tmp_path: Path) -> None:
        scaffold(tmp_path)
        content = (tmp_path / "config.toml").read_text()
    ...
    def test_creates_dest_dir_if_absent(self, tmp_path: Path) -> None:
        ...
        scaffold(dest)
        assert dest.is_dir()
        assert (dest / "compose.yaml").exists()
    ...
    def test_refuses_lists_conflicting_files(self, tmp_path: Path) -> None:
        (tmp_path / "compose.yaml").write_text("existing", encoding="utf-8")
        with pytest.raises(FileExistsError) as exc_info:
            scaffold(tmp_path)
        assert "compose.yaml" in str(exc_info.value)

    def test_force_overwrites_existing_files(self, tmp_path: Path) -> None:
        (tmp_path / "compose.yaml").write_text("old content", encoding="utf-8")
    ...
    def test_no_partial_write_on_collision(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(FileExistsError):
            scaffold(tmp_path)
        # compose.yaml should NOT have been written.
