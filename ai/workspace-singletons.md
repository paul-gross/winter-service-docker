...
- **Ports come from the sourced `.winter.workspace.env`** — workspace services get no `WSD_PORT_*` (that auto-derivation is a per-env convenience). Instead the orchestrator sources `<workspace>/.winter.workspace.env` (seeded by `winter ws init` with `WINTER_PORT_BASE` = the index-0 band, e.g. `4000`) before each compose call, so a workspace `compose.yaml` references `${WINTER_PORT_BASE}` directly — `ports: ["${WINTER_PORT_BASE}:5432"]` publishes on `4000`, never colliding with a feature env. See `winter-service-docker:/ai/provider-contract.md#env-file-sourcing` for the sourcing contract (incl. shell arithmetic for additional service ports).
