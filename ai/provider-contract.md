...
This is the recommended path for **workspace** services: they have no `WSD_PORT_*` (see below), so they reference `${WINTER_WORKSPACE_PORT_BASE}` (and any other vars) straight from the sourced `.winter.workspace.env`.
